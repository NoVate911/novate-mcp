...
- Кнопка «Открыть сайт» показывается только если в корне проекта есть
  обычный файл `index.html`. Скачивание проекта потоковое: `tar -czf -` пишет
  прямо в HTTP-ответ, поэтому не возвращайся к ожиданию полного архива или `/tmp`.
...
- Обычные настройки в панели показывают эффективное значение прямо в input: их
  можно дополнять, менять и очищать. Пустая строка в overrides.json — осознанное
  переопределение, а не fallback к .env; settings.py и settings.ts синхронны.
...
- Поиск и фильтры работают на клиенте по `data-filter-*`: сервер добавляет
  ...
  mtime и рекурсивный размер папок; папки скачиваются потоковым tar.gz.
...
- Настройки разделены на клиентские вкладки Telegram, Конфигурация, Доступ и
  безопасность, Бэкапы. Категория задаётся `EditableSetting.section`; после save/reset/generate
