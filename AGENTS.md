...
- После каждого набора изменений агент обязан предложить готовое описание коммита в формате Conventional Commits.
- Формат: `type(scope): краткое описание` на английском, в повелительном наклонении, без точки в конце.
- Допустимые типы: `feat`, `fix`, `docs`, `refactor`, `test`, `build`, `ci`, `chore`, `perf`, `revert`.
- Для несовместимых изменений добавляй `!` после type/scope и секцию `BREAKING CHANGE:` в теле.
- Если полезно, после заголовка добавляй короткое тело с причинами и списком существенных изменений.
