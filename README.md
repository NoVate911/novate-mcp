...
- 🔐 **Безопасность по кругу** — Bearer-токен для MCP, OIDC + AES-256-GCM-сессии для панели, изоляция папки, контейнеры не от root
...
MCP-клиент ──HTTPS + Bearer──┐
       ...
       11 инстр.)     просмотр сайтов)    вход через Telegram OIDC)
                             │
                             ▼
                   ./projects на сервере —
...
подтверди вход в приложении Telegram — готово. Зашифрованная cookie
(AES-256-GCM) живёт 7 дней. Доступ есть только у Telegram ID из `ALLOWED_TG_USERS`,
