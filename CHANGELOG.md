...
- Исправлена same-origin проверка запросов на установку версии за reverse proxy: учитываются настроенный домен и нормализованные forwarded-заголовки.

- Выровнены вертикальные отступы новых блоков мониторинга, ограниченных токенов и раздела версий.
- Поле названия ограниченного токена приведено к стилю остальных элементов панели, включая hover и focus-состояния.
- Оформлен выбор проекта при восстановлении бэкапа.
- Добавлен отступ между формой создания и списком ограниченных токенов.
...
- Удаление проектов из панели: кнопка-корзина, подтверждение, same-origin POST, Telegram alert и немедленная S3-сверка.
...
- Фиксированный предел S3 startup заменён на `NOVATE_S3_STALL_TIMEOUT`: дедлайн обновляется при каждом изменении фазы или счётчика прогресса.
